...
# Pestaña 8: Mapa desde un shapefile
with tab8:
    st.header('Mapa desde Shapefile')

    # Cargar el shapefile usando geopandas
    shape_file_path = 'ruta_al_shapefile.shp'  # Reemplaza con la ruta de tu shapefile
    gdf = gpd.read_file(shape_file_path)

    # Obtener el centro del mapa basado en la geometría del shapefile
    center = gdf.geometry.centroid.y.mean(), gdf.geometry.centroid.x.mean()

    # Crear un objeto de mapa de Folium
    mapa = folium.Map(location=center, zoom_start=12)

    # Agregar el shapefile al mapa usando GeoJson
